...
try {
    ...
    dotnet run --project (Join-Path $projectRoot 'tests\VelrixWorkHub.Workflow.PostgreSqlProbe\VelrixWorkHub.Workflow.PostgreSqlProbe.csproj') --configuration Debug -- --benchmark
    if ($LASTEXITCODE -ne 0) { throw "Workflow 数据库性能基准失败，退出码：$LASTEXITCODE" }
}
finally {
