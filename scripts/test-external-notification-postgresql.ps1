...
if ([string]::IsNullOrWhiteSpace($ConnectionString) -and $DatabaseType -eq 'PostgreSQL') {
    $settings = Get-Content -Raw (Join-Path $projectRoot 'src\VelrixWorkHub.Web\appsettings.json') | ConvertFrom-Json
...
try {
    ...
    dotnet run --project $probeProject --configuration Debug --artifacts-path (Join-Path $projectRoot 'artifacts\external-notification-postgresql-probe') -- --external-notification
    if ($LASTEXITCODE -ne 0) { throw "站外通知持久化探针失败，退出码：$LASTEXITCODE" }
}
finally {
