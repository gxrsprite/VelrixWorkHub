...
public sealed class PmsPortfolioServiceTests
{
    [Fact]
    public void Build_AggregatesWbsIssuesEvmAndReceivables()
    {
        ...
        var project = new PmsProject("PRJ-PORTFOLIO", "组合测试项目", customer.Id, "项目经理", today.AddDays(-5), today.AddDays(20));
        project.SetStatus(PmsProjectStatus.Active);
        var doneTask = new PmsWbsTask(project.Id, null, "已完成工作包", "成员 A", 1, today.AddDays(-3), today.AddDays(-2), false);
        doneTask.SetPercentComplete(100);
        var pendingTask = new PmsWbsTask(project.Id, null, "待完成工作包", "成员 B", 2, today, today.AddDays(2), false);
        var overdueMilestone = new PmsProjectPhase(project.Id, "逾期里程碑", PmsProjectPhaseKind.Milestone, 1, today.AddDays(-1), today.AddDays(-1));
        var overdueIssue = new PmsProjectIssue(project.Id, PmsProjectIssueKind.Risk, "逾期风险", null, "项目经理", PmsProjectIssuePriority.High, today.AddDays(-1));
        var closedIssue = new PmsProjectIssue(project.Id, PmsProjectIssueKind.Issue, "已关闭问题", null, null, PmsProjectIssuePriority.Low, today.AddDays(-1));
        closedIssue.SetStatus(PmsProjectIssueStatus.Closed);
        var evm = new PmsEvmSnapshot(project, null, 100, 80, 100);
        ...
        var result = PmsPortfolioService.Build(today, new[] { project }, new[] { overdueMilestone }, new[] { doneTask, pendingTask }, new[] { overdueIssue, closedIssue }, new[] { evm }, new[] { order }, new[] { receipt }, new[] { activeContract, terminatedContract });
        ...
        Assert.Equal(PmsDeliveryHealthStatus.AtRisk, item.DeliveryHealth.Status);
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today); var customer = new Customer("Aster 科技"); var product = new Product("SKU-PMS-SCOPE", "项目服务", "件", 100m, null);
        var first = new PmsProject("PRJ-SCOPE-01", "项目一", customer.Id, null, today, today.AddDays(30));
        var second = new PmsProject("PRJ-SCOPE-02", "项目二", customer.Id, null, today, today.AddDays(30));
        ...
        var result = PmsPortfolioService.Build(today, [first, second], [], [], [], [], [firstOrder, secondOrder, unassignedOrder], [firstReceipt, unassignedReceipt], [contract]);
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var attentionProject = new PmsProject("PRJ-HEALTH-ATTN", "需关注项目", null, null, today, today.AddDays(30));
        var openIssue = new PmsProjectIssue(attentionProject.Id, PmsProjectIssueKind.Risk, "待确认风险", null, null, PmsProjectIssuePriority.Medium, today.AddDays(5));
        var healthyProject = new PmsProject("PRJ-HEALTHY", "健康项目", null, null, today, today.AddDays(30));

        var result = PmsPortfolioService.Build(today, [attentionProject, healthyProject], [], [], [openIssue], [], [], []);

        Assert.Equal(1, result.AttentionProjectCount);
        Assert.Equal(0, result.AtRiskProjectCount);
        Assert.Equal(PmsDeliveryHealthStatus.Attention, result.Projects[0].DeliveryHealth.Status);
        Assert.Contains("未关闭风险问题", string.Join("、", result.Projects[0].DeliveryHealth.Reasons));
        Assert.Equal(PmsDeliveryHealthStatus.Healthy, result.Projects[1].DeliveryHealth.Status);
