...
public sealed class PmsProjectTests
{
    [Fact]
    public void Project_RequiresCodeNameAndValidPlanDates()
    {
        var project = new PmsProject(" PRJ-1 ", " 交付项目 ", null, " 项目经理 ", new DateOnly(2026, 7, 12), new DateOnly(2026, 8, 12));
        Assert.Equal("PRJ-1", project.Code);
        Assert.Equal("交付项目", project.Name);
        Assert.Throws<ArgumentException>(() => new PmsProject("PRJ-2", "项目", null, null, new DateOnly(2026, 8, 12), new DateOnly(2026, 7, 12)));
    }

    [Fact]
    public void Project_PercentCompleteIsBoundedAndCompletesActiveProject()
    {
        var project = new PmsProject("PRJ-1", "交付项目", null, null, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today.AddDays(1)));
        project.SetStatus(PmsProjectStatus.Active);
        project.SetPercentComplete(100);
        Assert.Equal(PmsProjectStatus.Completed, project.Status);
    ...
    public void Phase_RequiresProjectNameSequenceAndValidDates()
    {
        ...
        var phase = new PmsProjectPhase(projectId, " 方案确认 ", PmsProjectPhaseKind.Phase, 1, new DateOnly(2026, 7, 12), new DateOnly(2026, 7, 26));
        Assert.Equal("方案确认", phase.Name);
        Assert.Throws<ArgumentOutOfRangeException>(() => new PmsProjectPhase(projectId, "阶段", PmsProjectPhaseKind.Phase, 0, new DateOnly(2026, 7, 12), new DateOnly(2026, 7, 12)));
        Assert.Throws<ArgumentException>(() => new PmsProjectPhase(projectId, "阶段", PmsProjectPhaseKind.Phase, 1, new DateOnly(2026, 7, 26), new DateOnly(2026, 7, 12)));
    }

    [Fact]
    public void Milestone_AtFullPercentBecomesCompleted()
    {
        var milestone = new PmsProjectPhase(Guid.CreateVersion7(), "方案评审", PmsProjectPhaseKind.Milestone, 2, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today));
        milestone.SetPercentComplete(100);
        Assert.Equal(PmsProjectPhaseStatus.Completed, milestone.Status);
    }
    ...
    {
        var projectId = Guid.CreateVersion7();
        var task = new PmsWbsTask(projectId, null, " 需求确认 ", " 负责人 ", 1, new DateOnly(2026, 7, 12), new DateOnly(2026, 7, 19), false);
        Assert.Equal("需求确认", task.Title);
        Assert.Throws<ArgumentException>(() => new PmsWbsTask(projectId, null, "里程碑", null, 1, new DateOnly(2026, 7, 12), new DateOnly(2026, 7, 13), true));
        Assert.Throws<ArgumentOutOfRangeException>(() => new PmsWbsTask(projectId, null, "任务", null, 0, new DateOnly(2026, 7, 12), new DateOnly(2026, 7, 12), false));
    }

    [Fact]
    public void WbsTask_AtFullPercentBecomesDone()
    {
        var task = new PmsWbsTask(Guid.CreateVersion7(), null, "需求确认", null, 1, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today), true);
        task.SetPercentComplete(100);
        Assert.Equal(PmsWbsTaskStatus.Done, task.Status);
    }

    [Fact]
    public void ProjectMember_RequiresProjectMemberAndRole()
    {
        var member = new PmsProjectMember(Guid.CreateVersion7(), " 项目经理 ", " 项目主责人 ", true);
        Assert.Equal("项目经理", member.MemberName);
        Assert.Equal("项目主责人", member.RoleName);
        Assert.Throws<ArgumentException>(() => new PmsProjectMember(Guid.Empty, "成员", "角色"));
        Assert.Throws<ArgumentException>(() => new PmsProjectMember(member.ProjectId, "", "角色"));
    }

    [Fact]
    public void ProjectIssue_RequiresTitleAndNormalizesOptionalFields()
    {
        var issue = new PmsProjectIssue(Guid.CreateVersion7(), PmsProjectIssueKind.Risk, " 延期风险 ", " 描述 ", " 负责人 ", PmsProjectIssuePriority.High, new DateOnly(2026, 7, 19));
        Assert.Equal("延期风险", issue.Title);
        Assert.Equal("描述", issue.Description);
        Assert.Equal("负责人", issue.OwnerName);
        Assert.Throws<ArgumentException>(() => new PmsProjectIssue(issue.ProjectId, PmsProjectIssueKind.Issue, "", null, null, PmsProjectIssuePriority.Low, null));
    }
    ...
    {
        var projectId = Guid.CreateVersion7();
        var baseline = new PmsProjectBaseline(projectId, 1, " 立项基线 ", DateTime.Now, new DateOnly(2026, 7, 12), new DateOnly(2026, 8, 12), 25, 2, 5);
        Assert.Equal("立项基线", baseline.Label);
        Assert.Throws<ArgumentOutOfRangeException>(() => new PmsProjectBaseline(projectId, 0, "基线", DateTime.Now, new DateOnly(2026, 7, 12), new DateOnly(2026, 8, 12), 0, 0, 0));
        Assert.Throws<ArgumentException>(() => new PmsProjectBaseline(projectId, 1, "基线", DateTime.Now, new DateOnly(2026, 8, 12), new DateOnly(2026, 7, 12), 0, 0, 0));
    }

    [Fact]
    public void BaselineComparison_ReportsCurrentDeltas()
    {
        var baseline = new PmsProjectBaseline(Guid.CreateVersion7(), 1, "基线", DateTime.Now, new DateOnly(2026, 7, 12), new DateOnly(2026, 8, 12), 25, 2, 5);
        var project = new PmsProject("PRJ-1", "项目", null, null, new DateOnly(2026, 7, 12), new DateOnly(2026, 8, 19));
        project.SetPercentComplete(40);
        var comparison = new VelrixWorkHub.Application.PmsProjects.PmsBaselineComparison(baseline, project, 3, 7);
    ...
    {
        var change = new PmsProjectChange(Guid.CreateVersion7(), " 补充范围 ", " 客户新增需求 ", " 增加工期 ", " 申请人 ", DateTime.Now);
        Assert.Equal("补充范围", change.Title);
        Assert.Equal(PmsProjectChangeStatus.Proposed, change.Status);
        Assert.Throws<ArgumentException>(() => new PmsProjectChange(change.ProjectId, "", "原因", null, null, DateTime.Now));
        Assert.Throws<ArgumentException>(() => new PmsProjectChange(change.ProjectId, "标题", "", null, null, DateTime.Now));
    }

    [Fact]
    public void WorkLog_RequiresMemberAndBoundsHours()
    {
        var log = new PmsWorkLog(Guid.CreateVersion7(), null, DateOnly.FromDateTime(DateTime.Today), " 项目经理 ", 6.567m, " 需求确认 ");
        Assert.Equal("项目经理", log.MemberName);
        Assert.Equal(6.57m, log.Hours);
        Assert.Throws<ArgumentOutOfRangeException>(() => new PmsWorkLog(log.ProjectId, null, log.WorkDate, "成员", 24.1m, null));
        Assert.Throws<ArgumentException>(() => new PmsWorkLog(log.ProjectId, null, log.WorkDate, "", 1m, null));
    }

    [Fact]
    public void EvmSnapshot_CalculatesPerformanceIndexes()
    {
        var project = new PmsProject("PRJ-1", "项目", null, null, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today.AddDays(1)));
        var snapshot = new VelrixWorkHub.Application.PmsProjects.PmsEvmSnapshot(project, null, 16m, 8m, 10m);
