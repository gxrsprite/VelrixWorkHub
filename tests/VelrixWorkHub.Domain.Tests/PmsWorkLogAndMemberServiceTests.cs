...
public sealed class PmsWorkLogAndMemberServiceTests
{
    [Fact]
    public void WorkLogService_RequiresProjectMemberAndProjectDateRange()
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-WORKLOG", "工时一致性项目", null, null, today, today.AddDays(10));
        var member = new PmsProjectMember(project.Id, "项目经理", "项目主责人", true);
        var task = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var logs = new WorkLogRepository();
        var service = new PmsWorkLogService(logs, new ProjectRepository(project), new WbsRepository(task), new MemberRepository(member));
    ...
    {
        var project = new PmsProject("PRJ-MEMBER", "成员治理项目", null, null, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today.AddDays(10)));
        var primary = new PmsProjectMember(project.Id, "项目经理", "项目主责人", true);
        var second = new PmsProjectMember(project.Id, "业务负责人", "业务负责人");
        var members = new MemberRepository(primary, second);
        var service = new PmsProjectMemberService(members, new ProjectRepository(project));
    ...
    {
        var project = new PmsProject("PRJ-DIRECTORY", "目录成员项目", null, null, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today.AddDays(10)));
        ...
        var service = new PmsProjectMemberService(members, new ProjectRepository(project), new EmployeeDirectoryService(new DirectoryRepository(enabled, disabled)));
    ...
    {
        var project = new PmsProject("PRJ-DIRECTORY-NAME", "目录重名项目", null, null, DateOnly.FromDateTime(DateTime.Today), DateOnly.FromDateTime(DateTime.Today.AddDays(10)));
        ...
        var service = new PmsProjectMemberService(new MemberRepository(), new ProjectRepository(project), new EmployeeDirectoryService(new DirectoryRepository(first, second)));
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var mine = new PmsProject("PRJ-VISIBLE-MINE", "我的项目", null, null, today, today.AddDays(10));
        var other = new PmsProject("PRJ-VISIBLE-OTHER", "其他项目", null, null, today, today.AddDays(10));
        var userId = Guid.CreateVersion7();
        var service = new PmsProjectMemberService(
            new MemberRepository(
                new PmsProjectMember(mine.Id, "项目成员", "实施", userId: userId),
                new PmsProjectMember(other.Id, "其他成员", "实施", userId: Guid.CreateVersion7()),
                new PmsProjectMember(other.Id, "项目成员", "历史同名", userId: Guid.CreateVersion7())),
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-AMBIGUOUS-VISIBILITY", "歧义可见性项目", null, null, today, today.AddDays(10));
        var userId = Guid.CreateVersion7();
        var service = new PmsProjectMemberService(
            new MemberRepository(
                new PmsProjectMember(project.Id, "历史成员甲", "实施", userId: userId),
                new PmsProjectMember(project.Id, "历史成员乙", "实施", userId: userId)),
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-MATRIX", "矩阵项目", null, null, today, today.AddDays(10));
        var member = new PmsProjectMember(project.Id, "项目经理", "项目主责人");
        var task = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var logs = new WorkLogRepository();
        var service = new PmsWorkLogService(logs, new ProjectRepository(project), new WbsRepository(task), new MemberRepository(member));

        service.SaveCell(project.Id, task.Id, today, member.MemberName, 4.5m, PmsWorkLogAttendanceStatus.NoAttendance, "补录");
        service.SaveCell(project.Id, task.Id, today, member.MemberName, 6m, PmsWorkLogAttendanceStatus.Normal, "修正");
        Assert.Equal(6m, Assert.Single(logs.List(project.Id)).Hours);
        Assert.Equal(PmsWorkLogAttendanceStatus.Normal, logs.List(project.Id).Single().AttendanceStatus);

        service.SaveCell(project.Id, task.Id, today, member.MemberName, 0, PmsWorkLogAttendanceStatus.Normal, null);
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-DAILY-HOURS", "日工时门禁项目", null, null, today, today.AddDays(10));
        var member = new PmsProjectMember(project.Id, "项目经理", "项目主责人");
        var firstTask = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var secondTask = new PmsWbsTask(project.Id, null, "方案评审", "项目经理", 2, today, today.AddDays(5), false);
        var logs = new WorkLogRepository();
        var service = new PmsWorkLogService(logs, new ProjectRepository(project), new WbsRepository(firstTask, secondTask), new MemberRepository(member));

        service.SaveCell(project.Id, firstTask.Id, today, member.MemberName, 14m, PmsWorkLogAttendanceStatus.Normal, null);
        service.SaveCell(project.Id, secondTask.Id, today, member.MemberName, 10m, PmsWorkLogAttendanceStatus.Normal, null);

        Assert.Throws<InvalidOperationException>(() => service.SaveCell(project.Id, secondTask.Id, today, member.MemberName, 10.1m, PmsWorkLogAttendanceStatus.Normal, null));
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-BATCH-HOURS", "批量工时门禁项目", null, null, today, today.AddDays(10));
        var member = new PmsProjectMember(project.Id, "项目经理", "项目主责人");
        var firstTask = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var secondTask = new PmsWbsTask(project.Id, null, "方案评审", "项目经理", 2, today, today.AddDays(5), false);
        var logs = new WorkLogRepository();
        var service = new PmsWorkLogService(logs, new ProjectRepository(project), new WbsRepository(firstTask, secondTask), new MemberRepository(member));
        service.SaveCell(project.Id, firstTask.Id, today, member.MemberName, 10m, PmsWorkLogAttendanceStatus.Normal, "原始工时");

        Assert.Throws<InvalidOperationException>(() => service.SaveCells(project.Id,
        [
            new PmsWorkLogCellSave(firstTask.Id, today, member.MemberName, 14m, PmsWorkLogAttendanceStatus.Normal, "待保存修改"),
            new PmsWorkLogCellSave(secondTask.Id, today, member.MemberName, 11m, PmsWorkLogAttendanceStatus.Normal, "超出累计")
        ]));
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-OWN-WORKLOG", "本人填报门禁项目", null, null, today, today.AddDays(10));
        var owner = new PmsProjectMember(project.Id, "项目经理", "项目主责人");
        var other = new PmsProjectMember(project.Id, "实施顾问", "交付成员");
        var task = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var service = new PmsWorkLogService(new WorkLogRepository(), new ProjectRepository(project), new WbsRepository(task), new MemberRepository(owner, other));

        Assert.Throws<UnauthorizedAccessException>(() => service.SaveCellsForMember(project.Id, owner.MemberName,
            [new PmsWorkLogCellSave(task.Id, today, other.MemberName, 8m, PmsWorkLogAttendanceStatus.Normal, null)]));
    }
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-STABLE-WORKLOG", "稳定身份工时项目", null, null, today, today.AddDays(10));
        var userId = Guid.CreateVersion7();
        var member = new PmsProjectMember(project.Id, "项目经理", "项目主责人", userId: userId);
        var task = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var logs = new WorkLogRepository();
        var service = new PmsWorkLogService(logs, new ProjectRepository(project), new WbsRepository(task), new MemberRepository(member));

        Assert.Throws<UnauthorizedAccessException>(() => service.SaveCellsForProjectMember(project.Id, Guid.CreateVersion7(), [new PmsWorkLogCellSave(task.Id, today, member.MemberName, 8m, PmsWorkLogAttendanceStatus.Normal, null)]));
        service.SaveCellsForProjectMember(project.Id, userId, [new PmsWorkLogCellSave(task.Id, today, member.MemberName, 8m, PmsWorkLogAttendanceStatus.Normal, null)]);
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-READ-SCOPE", "工时读取范围项目", null, null, today, today.AddDays(10));
        var userId = Guid.CreateVersion7();
        var mine = new PmsProjectMember(project.Id, "我的成员", "实施", userId: userId);
        var other = new PmsProjectMember(project.Id, "其他成员", "实施", userId: Guid.CreateVersion7());
        var logs = new WorkLogRepository(
            new PmsWorkLog(project.Id, null, today, mine.MemberName, 8m, null),
            new PmsWorkLog(project.Id, null, today, other.MemberName, 6m, null));
        var service = new PmsWorkLogService(logs, new ProjectRepository(project), new WbsRepository(), new MemberRepository(mine, other));
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-AMBIGUOUS-MEMBER", "歧义成员项目", null, null, today, today.AddDays(10));
        var userId = Guid.CreateVersion7();
        var first = new PmsProjectMember(project.Id, "历史成员甲", "实施", userId: userId);
        var second = new PmsProjectMember(project.Id, "历史成员乙", "实施", userId: userId);
        var service = new PmsWorkLogService(
            new WorkLogRepository(new PmsWorkLog(project.Id, null, today, first.MemberName, 8m, null)),
        ...
        Assert.Throws<UnauthorizedAccessException>(() => service.SaveCellsForProjectMember(project.Id, userId,
            [new PmsWorkLogCellSave(null, today, first.MemberName, 8m, PmsWorkLogAttendanceStatus.Normal, null)]));
    }
    ...
    {
        var today = DateOnly.FromDateTime(DateTime.Today);
        var project = new PmsProject("PRJ-BATCH-TX", "批量工时事务项目", null, null, today, today.AddDays(10));
        var member = new PmsProjectMember(project.Id, "项目经理", "项目主责人");
        var task = new PmsWbsTask(project.Id, null, "需求确认", "项目经理", 1, today, today.AddDays(5), false);
        var boundary = new RecordingTransactionBoundary();
        var service = new PmsWorkLogService(new WorkLogRepository(), new ProjectRepository(project), new WbsRepository(task), new MemberRepository(member), boundary);

        service.SaveCells(project.Id, [new PmsWorkLogCellSave(task.Id, today, member.MemberName, 8m, PmsWorkLogAttendanceStatus.Normal, null)]);
