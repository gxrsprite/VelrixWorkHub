...
public sealed class PmpProjectWorkItemReminderServiceTests
{
    [Fact]
    public void Scan_NotifiesDueOpenItemsOnceAndSkipsTerminalOrUnassignedItems()
    {
        ...
        completed.SetStatus(PmpProjectWorkItemStatus.InProgress, null, now.AddHours(-1));
        completed.SetStatus(PmpProjectWorkItemStatus.PendingApproval, "已完成", now.AddMinutes(-1));
        ...
        var service = new PmpProjectWorkItemReminderService(
            items,
    ...
    public void Scan_NotifiesOverdueItemsOnceAndSkipsTerminalOrUnavailableOwners()
    {
        ...
        inApproval.SetStatus(PmpProjectWorkItemStatus.Open, null, now.AddHours(-2));
        inApproval.SetStatus(PmpProjectWorkItemStatus.InProgress, null, now.AddHours(-1));
        inApproval.SetStatus(PmpProjectWorkItemStatus.PendingApproval, "待验收", now.AddMinutes(-30));
        var completed = AddItem(items, "已完成逾期", owner.UserId, reminderAt: null, plannedEndAt: now.AddMinutes(-3));
        completed.SetStatus(PmpProjectWorkItemStatus.Open, null, now.AddHours(-2));
        completed.SetStatus(PmpProjectWorkItemStatus.InProgress, null, now.AddHours(-1));
        completed.SetStatus(PmpProjectWorkItemStatus.PendingApproval, "已完成", now.AddMinutes(-30));
        ...
        var notifications = new NotificationRepository();
        var service = new PmpProjectWorkItemReminderService(items, new EmployeeDirectoryService(new DirectoryRepository(owner, disabledOwner)), new NotificationService(notifications));
        ...
        Assert.All(notifications.Items, notification =>
        {
            ...
            Assert.StartsWith("pmp-work-item-overdue:", notification.DedupeKey, StringComparison.Ordinal);
        });
    ...
    public void Scan_KeepsScheduledAndOverdueNotificationsSeparate()
    {
        ...
        var notifications = new NotificationRepository();
        var service = new PmpProjectWorkItemReminderService(items, new EmployeeDirectoryService(new DirectoryRepository(owner)), new NotificationService(notifications));
    ...
    private static PmpProjectWorkItem AddItem(WorkItemRepository repository, string title, Guid? ownerId, DateTime? reminderAt, DateTime? plannedEndAt = null)
    {
        var item = new PmpProjectWorkItem(Guid.CreateVersion7(), null, null, null, title, null, ownerId is null ? null : "负责人", null, PmpProjectWorkItemPriority.Medium, null, plannedEndAt, "{}", ownerId, reminderAt);
        item.SetStatus(PmpProjectWorkItemStatus.Open, null, (reminderAt ?? plannedEndAt ?? DateTime.Now).AddHours(-1));
        repository.Add(item);
        return item;
    }
