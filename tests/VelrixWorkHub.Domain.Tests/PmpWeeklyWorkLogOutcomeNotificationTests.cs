...
public sealed class PmpWeeklyWorkLogOutcomeNotificationTests
{
    [Fact]
    public void WorkflowHandler_NotifiesEnabledProjectMemberWithApprovalResult()
    {
        ...
        var item = new PmpWeeklyWorkLogSubmission(projectId, "项目经理", new DateOnly(2026, 7, 20), "[{\"hours\":8}]", 8m);
        item.Submit("project.manager", DateTime.Now);
        var notifications = new NotificationRepository();
        var handler = new PmpWeeklyWorkLogSubmissionWorkflowActionHandler(
            new SubmissionRepository(item),
            outcomeNotifications: new PmpWeeklyWorkLogOutcomeNotificationService(
                new MemberRepository(new PmpProjectMember(projectId, item.MemberName, "项目经理", userId: userId)),
        ...
        handler.Execute(new WorkflowActionContext(instance, WorkflowActionTrigger.Approved, null), new WorkflowActionDefinition(WorkflowActionType.SetField, nameof(PmpWeeklyWorkLogSubmission.Status), nameof(PmpWeeklyWorkLogSubmissionStatus.Approved)));
    ...
    {
        var item = new PmpWeeklyWorkLogSubmission(Guid.CreateVersion7(), "项目经理", new DateOnly(2026, 7, 20), "[{\"hours\":8}]", 8m);
        var notifications = new NotificationRepository();
        var service = new PmpWeeklyWorkLogOutcomeNotificationService(new MemberRepository(), new EmployeeDirectoryService(new DirectoryRepository()), new NotificationService(notifications));
        ...
        var ambiguousService = new PmpWeeklyWorkLogOutcomeNotificationService(
            new MemberRepository(
                new PmpProjectMember(item.ProjectId, item.MemberName, "角色一", userId: firstUserId),
                new PmpProjectMember(item.ProjectId, item.MemberName, "角色二", userId: secondUserId)),
    ...
    {
        var definition = new WorkflowDefinition(WorkflowBindingCodes.PmpWeeklyWorkLogApproval, "周工时审批");
        ...
        return WorkflowInstance.Start(definition, nameof(PmpWeeklyWorkLogSubmission), businessId, DateTime.Now);
    }

    private sealed class SubmissionRepository(params PmpWeeklyWorkLogSubmission[] items) : IPmpWeeklyWorkLogSubmissionRepository { private readonly List<PmpWeeklyWorkLogSubmission> data = [.. items]; public IReadOnlyList<PmpWeeklyWorkLogSubmission> List(Guid? projectId = null) => projectId is Guid id ? data.Where(x => x.ProjectId == id).ToArray() : data; public void Add(PmpWeeklyWorkLogSubmission item) => data.Add(item); public void Update(PmpWeeklyWorkLogSubmission item) { } public void Remove(Guid id) => data.RemoveAll(x => x.Id == id); }
    private sealed class MemberRepository(params PmpProjectMember[] items) : IPmpProjectMemberRepository { public IReadOnlyList<PmpProjectMember> List(Guid? projectId = null) => projectId is Guid id ? items.Where(x => x.ProjectId == id).ToArray() : items; public void Add(PmpProjectMember item) { } public void Update(PmpProjectMember item) { } public void Remove(Guid id) { } }
