...
public enum PmpProjectStatus { Draft, Active, OnHold, Completed, Cancelled }
public enum PmpProjectInitiationMode { PreInitiation, FormalInitiation }

public sealed class PmpProject
{
    ...
    public PmpProjectInitiationMode InitiationMode { get; private set; }
    ...
    public PmpProjectStatus Status { get; private set; }

    public PmpProject(string code, string name, Guid? customerId, string? managerName, DateOnly plannedStart, DateOnly plannedEnd)
        : this(code, name, customerId, managerName, plannedStart, plannedEnd, PmpProjectInitiationMode.PreInitiation, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null)
    {
    }

    public PmpProject(string code, string name, Guid? customerId, string? managerName, DateOnly plannedStart, DateOnly plannedEnd,
        PmpProjectInitiationMode initiationMode, string? projectAlias, string? projectChineseName, string? projectEnglishName,
    ...
    public void EditDetails(PmpProjectInitiationMode initiationMode, string? projectAlias, string? projectChineseName, string? projectEnglishName,
        string? productName, string? projectStage, string? productLine, string? projectCategory, string? projectSubcategory,
        string? projectSubcategoryCode, string? versionType, string? projectVersion, DateOnly? expectedInitiationDate,
        DateOnly? actualInitiationDate, string? developmentMode, string? departmentName, string? domainManagerName,
        string? businessInitiatorName, string? overview, string? objective, string? otherInfo)
    ...
    public void SetStatus(PmpProjectStatus status) => Status = status;
    public void SetPercentComplete(int percent)
    {
        if (percent is < 0 or > 100) throw new ArgumentOutOfRangeException(nameof(percent), "完成百分比必须在 0 到 100 之间。");
        PercentComplete = percent;
        if (percent == 100 && Status == PmpProjectStatus.Active) Status = PmpProjectStatus.Completed;
    }

    private static string? Clean(string? value) => string.IsNullOrWhiteSpace(value) ? null : value.Trim();
}

public sealed class PmpProjectStatusHistory
{
    public Guid Id { get; init; } = Guid.CreateVersion7();
    public Guid ProjectId { get; private set; }
    public PmpProjectStatus FromStatus { get; private set; }
    public PmpProjectStatus ToStatus { get; private set; }
    public string Reason { get; private set; } = string.Empty;
    public string ActorName { get; private set; } = string.Empty;
    public DateTime ChangedAt { get; private set; }

    public PmpProjectStatusHistory(Guid projectId, PmpProjectStatus fromStatus, PmpProjectStatus toStatus, string reason, string actorName, DateTime changedAt)
    {
