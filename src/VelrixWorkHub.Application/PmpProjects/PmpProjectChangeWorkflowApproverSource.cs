...
public sealed class PmpProjectChangeWorkflowApproverSource(IPmpProjectChangeRepository repository) : IWorkflowBusinessApproverSource
{
    public bool CanHandle(WorkflowInstance instance) => instance.BusinessType.Equals(nameof(PmpProjectChange), StringComparison.OrdinalIgnoreCase);
    ...
    public IReadOnlyList<string> FindUsernames(WorkflowInstance instance, IReadOnlyCollection<string> fieldNames)
    {
        ...
        {
            if (fieldName.Equals(nameof(PmpProjectChange.RequesterName), StringComparison.OrdinalIgnoreCase))
