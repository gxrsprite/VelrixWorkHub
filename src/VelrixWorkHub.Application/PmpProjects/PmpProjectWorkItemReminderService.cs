...
public sealed class PmpProjectWorkItemReminderService(
    IPmpProjectWorkItemRepository repository,
    EmployeeDirectoryService directory,
    NotificationService notifications)
{
    public PmpProjectWorkItemReminderScanResult Scan(DateTime now)
    {
        ...
        {
            if (item.Status is PmpProjectWorkItemStatus.Completed or PmpProjectWorkItemStatus.Cancelled)
            {
                skipped++;
                continue;
            }
        ...
        return new PmpProjectWorkItemReminderScanResult(due, overdue, delivered, skipped);
    }

    private static string ReminderDedupeKey(PmpProjectWorkItem item, DateTime reminderAt) =>
        $"pmp-work-item-reminder:{item.Id}:{reminderAt.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture)}";

    private static string OverdueDedupeKey(PmpProjectWorkItem item, DateTime plannedEndAt) =>
        $"pmp-work-item-overdue:{item.Id}:{plannedEndAt.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture)}";
}

public sealed record PmpProjectWorkItemReminderScanResult(int DueWorkItemCount, int OverdueWorkItemCount, int NotificationAttemptCount, int SkippedWorkItemCount);
