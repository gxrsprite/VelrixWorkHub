...
public sealed class PmpWbsTaskService(IPmpWbsTaskRepository repository, IPmpProjectRepository projectRepository)
{
    public IReadOnlyList<PmpWbsTask> List(Guid? projectId = null, string? keyword = null)
    {
    ...
    public PmpWbsTask Create(Guid projectId, Guid? parentId, string title, string? assignee, int sequence, DateOnly start, DateOnly end, bool milestone)
    {
        var project = EnsureProject(projectId); EnsureWithinProject(project, start, end); EnsureParent(projectId, parentId, null, start, end);
        var item = new PmpWbsTask(projectId, parentId, title, assignee, sequence, start, end, milestone); EnsureUnique(item); repository.Add(item); return item;
    }

    public void Edit(PmpWbsTask item, Guid? parentId, string title, string? assignee, int sequence, DateOnly start, DateOnly end, bool milestone)
    {
    ...
    public void SetStatus(PmpWbsTask item, PmpWbsTaskStatus status) { item.SetStatus(status); repository.Update(item); }
    public void SetPercentComplete(PmpWbsTask item, int percent) { item.SetPercentComplete(percent); repository.Update(item); }
    public void Remove(PmpWbsTask item)
    {
        foreach (var child in repository.List(item.ProjectId).Where(x => x.ParentId == item.Id).ToArray()) Remove(child);
        repository.Remove(item.Id);
    }

    private PmpProject EnsureProject(Guid projectId) => projectRepository.List().FirstOrDefault(x => x.Id == projectId) ?? throw new InvalidOperationException("关联项目不存在。");
    ...
    private void EnsureChildrenWithinRange(PmpWbsTask item, DateOnly start, DateOnly end)
    {
    ...
    private void EnsureUnique(PmpWbsTask item) => EnsureUnique(item.ProjectId, item.Id, item.ParentId, item.Title);
