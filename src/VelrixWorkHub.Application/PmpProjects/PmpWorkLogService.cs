...
public sealed record PmpWorkLogCellSave(Guid? TaskId, DateOnly Date, string MemberName, decimal Hours, PmpWorkLogAttendanceStatus AttendanceStatus, string? Note);
    ...
    IWorkflowTransactionBoundary? transactions = null)
{
    ...
    public IReadOnlyList<PmpWorkLog> ListForProjectMember(Guid projectId, Guid userId)
    {
    ...
    public IReadOnlyList<PmpWorkLog?> SaveCells(Guid projectId, IReadOnlyCollection<PmpWorkLogCellSave> cells)
    {
        if (cells.Count == 0) return [];
        EnsureBatchCanBeSaved(projectId, cells);
        IReadOnlyList<PmpWorkLog?> saved = [];
    ...
    public IReadOnlyList<PmpWorkLog?> SaveCellsForMember(Guid projectId, string memberName, IReadOnlyCollection<PmpWorkLogCellSave> cells)
    {
    ...
    public IReadOnlyList<PmpWorkLog?> SaveCellsForProjectMember(Guid projectId, Guid userId, IReadOnlyCollection<PmpWorkLogCellSave> cells)
    {
        var member = FindUniqueProjectMember(projectId, userId);
    ...
    private void EnsureBatchCanBeSaved(Guid projectId, IReadOnlyCollection<PmpWorkLogCellSave> cells)
    {
        var project = EnsureProject(projectId);
