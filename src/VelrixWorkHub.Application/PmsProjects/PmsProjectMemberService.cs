...
public sealed class PmsProjectMemberService(IPmsProjectMemberRepository repository, IPmsProjectRepository projectRepository, EmployeeDirectoryService? directory = null)
{
    public IReadOnlyList<PmsProjectMember> List(Guid? projectId = null, string? keyword = null)
    {
    ...
    public IReadOnlyList<PmsProject> ListProjectsForMember(Guid userId)
    {
        if (userId == Guid.Empty) return Array.Empty<PmsProject>();
    ...
    public PmsProjectMember? FindUniqueProjectMember(Guid projectId, Guid userId)
    {
    ...
    public PmsProjectMember Create(Guid projectId, string memberName, string roleName, bool isPrimary, string? departmentName = null)
    {
        EnsureProject(projectId); var item = new PmsProjectMember(projectId, memberName, roleName, isPrimary, departmentName); EnsureUnique(item); repository.Add(item); return item;
    }

    public PmsProjectMember CreateForPerson(Guid projectId, Guid userId, string roleName, bool isPrimary)
    {
        EnsureProject(projectId);
        var person = ResolveEnabledPerson(userId);
        var item = new PmsProjectMember(projectId, person.DisplayName, roleName, isPrimary, person.OrganizationName, person.UserId);
        EnsureUnique(item);
        repository.Add(item);
        return item;
    }

    public void Edit(PmsProjectMember item, string memberName, string roleName, string? departmentName = null)
    {
    ...
    public void EditForPerson(PmsProjectMember item, Guid userId, string roleName)
    {
        var person = ResolveEnabledPerson(userId);
    ...
    public void SetPrimary(PmsProjectMember item, bool value)
    {
    ...
    public void Remove(PmsProjectMember item) => repository.Remove(item.Id);
    ...
    private void EnsureUnique(PmsProjectMember item) => EnsureUnique(item.ProjectId, item.Id, item.UserId, item.MemberName);
