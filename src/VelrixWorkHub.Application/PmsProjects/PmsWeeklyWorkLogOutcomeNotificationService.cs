...
public sealed class PmsWeeklyWorkLogOutcomeNotificationService(
    IPmsProjectMemberRepository members,
    EmployeeDirectoryService directory,
    NotificationService notifications)
{
    public void Publish(PmsWeeklyWorkLogSubmission item, Guid workflowInstanceId, string target, string? reason = null)
    {
        ...
        notifications.Publish(
            ...
            content,
            "/Pms/WorkLog",
            $"pms-weekly-worklog-outcome:{item.Id}:{workflowInstanceId}:{target}");
    }
}
