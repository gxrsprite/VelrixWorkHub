...
public sealed record PmsProjectCommercialInsight(
...
public sealed record PmsProjectFulfillmentOrder(
    SalesOrder Order,
    decimal ReceivedAmount)
{
...
public static class PmsProjectCommercialInsightService
{
    public static PmsProjectCommercialInsight Build(
        PmsProject project,
        ...
        IEnumerable<SalesContract> contracts)
    {
        ...
        var hasAnyProjectAssignments = customerOrders.Any(x => x.PmsProjectId is not null && x.Status != SalesOrderStatus.Cancelled);
        var hasProjectScopedOrders = project.CustomerId is not null && customerOrders.Any(x => x.PmsProjectId == project.Id && x.Status != SalesOrderStatus.Cancelled);
        ...
        return new PmsProjectCommercialInsight(
    ...
    public static PmsProjectFulfillmentOrder[] Orders(PmsProject project, IEnumerable<SalesOrder> salesOrders, IEnumerable<ErpSettlement> settlements)
    {
        var customerOrders = CustomerOrders(project, salesOrders);
        var hasAnyProjectAssignments = customerOrders.Any(x => x.PmsProjectId is not null && x.Status != SalesOrderStatus.Cancelled);
        var scopedOrders = ScopeOrders(project, customerOrders, hasAnyProjectAssignments)
        ...
        return scopedOrders.Select(order => new PmsProjectFulfillmentOrder(order, receivedByOrder.GetValueOrDefault(order.Id))).ToArray();
    }

    private static SalesOrder[] CustomerOrders(PmsProject project, IEnumerable<SalesOrder> salesOrders) => project.CustomerId is Guid customerId
        ? salesOrders.Where(x => x.CustomerId == customerId).ToArray()
        : [];

    private static SalesOrder[] ScopeOrders(PmsProject project, IReadOnlyList<SalesOrder> customerOrders, bool hasAnyProjectAssignments) => hasAnyProjectAssignments
        ? customerOrders.Where(x => x.PmsProjectId == project.Id).ToArray()
