...
public sealed class PurchaseOrderService(IPurchaseOrderRepository repository, ISupplierRepository supplierRepository, IProductRepository productRepository, IInventoryTransactionRepository inventoryRepository, IWarehouseRepository warehouseRepository, ISettlementRepository settlementRepository, WorkflowApprovalService? approval = null, IOaProcurementRequestRepository? procurementRequests = null, ProcurementBudgetService? procurementBudgets = null, IWorkflowTransactionBoundary? transactions = null) : IPurchaseOrderWorkflowApprover
{
    ...
    public void Receive(PurchaseOrder item)
    {
        if (item.Status != PurchaseOrderStatus.Submitted) throw new InvalidOperationException("只有已提交的采购订单可以收货。");
        var warehouse = warehouseRepository.List().FirstOrDefault(x => x.Status == WarehouseStatus.Active) ?? throw new InvalidOperationException("没有可用的启用仓库。");
        ...
        void Core()
        {
            ...
            repository.Update(item);
            inventoryRepository.Add(new InventoryTransaction(item.ProductId, warehouse.Id, InventoryTransactionKind.Inbound, item.Quantity, sourceNo, item.OrderDate, $"采购订单 {item.OrderNo} 收货入库"));
        }
